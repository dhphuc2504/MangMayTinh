...
std::vector<fs::path> get_com_dirs() {
...
std::vector<DWORD> find_process_ids(const std::string& input_name) {
    std::string target = to_Low(input_name);
    if (target.size() >= 4) {
    ...
    if (Process32FirstW(snapshot, &pe32)) {
        do {
            std::string proc_name = to_Low(std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(pe32.szExeFile));
